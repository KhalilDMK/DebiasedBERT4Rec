...
class BertDataloader(AbstractDataloader):
    ...
    def get_train_temporal_popularity(self):
        #position_distribution = PositionDistribution(self.train, self.max_len, self.item_count)
...
#class PositionDistribution(data_utils.Dataset):
#    def __init__(self, u2seq, max_len, item_count):
#        self.item_count = item_count
#        self.u2seq = u2seq
#        self.items = list(range(1, self.item_count + 1))
#        #self.max_len = max_len
...
#        item_index = pd.Index(range(self.item_count + 1))
#        occurrences = pd.DataFrame(occurrences).transpose().reindex(item_index).sort_index().fillna(0).values[1::]
